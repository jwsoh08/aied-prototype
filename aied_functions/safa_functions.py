...
SAFA_PROMPT_OPTIONS = config_handler.get_config_values('menu_lists', 'SAFA_PROMPT_OPTIONS')
SAFA_TOOLS_OPTIONS = config_handler.get_config_values('menu_lists', 'SAFA_TOOLS_OPTIONS')
SAFA_MODEL_LIST = config_handler.get_config_values('menu_lists', 'SAFA_MODEL_LIST')
SAFA_CONFIG = config_handler.get_config_values('menu_lists', 'SAFA_CONFIG')
...
def chatbot_settings():
	...
	if st.checkbox("Revert to ShortAns FA default settings"):
		...
		st.session_state.default_presence_penalty = SAFA_CONFIG['default_presence_penalty']
		st.session_state.default_frequency_penalty = SAFA_CONFIG['default_frequency_penalty']
	...
	presence_penalty = st.number_input("Presence Penalty (OpenAI LLM)", value=st.session_state.default_presence_penalty, min_value=-2.0, max_value=2.0, step=0.1)
	frequency_penalty = st.number_input("Frequency Penalty (OpenAI LLM)", value=st.session_state.default_frequency_penalty, min_value=-2.0, max_value=2.0, step=0.1)
	...
	if st.checkbox("Use Seed Number"):
		...
		seed_number = st.number_input("Seed Number (OpenAI LLM)", value=st.session_state.default_seed_number, min_value=0, max_value=100000, step=1)
	else:
		st.session_state.use_seed = False
		seed_number = 1
...
def prompt_analyser():
	...
	with a1:
	 # Find the index of the default model
		...
		select_model = st.selectbox("Select a model", SAFA_MODEL_LIST, index=default_index)
	with a2:
		leniency_settings()
	with a3:
		pass
	...
	if selected_option == 'Single API call (Text)':
		...
		if content_prompt and st.session_state.j_tools_format:
			...
			if select_model.startswith("claude"):
				claude_api_call_json(content_prompt[1], select_model, tools, content_prompt[0])
			else:
...
@st.experimental_dialog("Update the Rubrics")
def rubrics_dialog():
	st.write(st.session_state.rubrics_prompt)
	rubrics = st.text_area("Rubrics", value=st.session_state.rubrics_prompt, max_chars=5000, height=200)
...
def short_answer_fa_interface(json_call):
	...
	with c1:
		pass_level = st.selectbox("Select your level:", options=LEVELS, index=st.session_state.pass_level)
	with c2:
		pass_subject = st.selectbox("Selecte your subject:", options=SUBJECTS, index=st.session_state.pass_subject)
	...
	if json_call:
		if st.button("Submit Content Prompt for JSON call", key = 1):
			if not st.session_state.prompt_design:
				...
				st.session_state.pass_level = LEVELS.index(pass_level)
				st.session_state.pass_subject = SUBJECTS.index(pass_subject)
...
def claude_api_batch_call_json(full_prompt, model, tools, rubrics):
	#openai.api_key = return_openai_key()
	#os.environ["OPENAI_API_KEY"] = return_openai_key()
	#st.title("Api Call with JSON")
	#full_prompt = str(full_prompt)
	...
	if response:  # Check if response is valid
		...
		duration = (end_time - start_time).total_seconds()  # Calculate duration in seconds
		if rubrics is not None:
			marks, feedback, extracted_rubrics = parse_claude_json_rubrics_data(response)
		else:
