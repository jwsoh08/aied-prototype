...
CHATBOT_MODEL_LIST = config_handler.get_config_values('menu_lists', 'CHATBOT_MODEL_LIST')
...
def bot_settings():
	with st.form(key='sliders_form'):
		...
		print(type(st.session_state.default_temp))
		temp = st.slider("Temp", min_value=0.0, max_value=1.0, value=float(st.session_state.default_temp), step=0.01)
		presence_penalty = st.slider("Presence Penalty", min_value=-2.0, max_value=2.0, value=float(st.session_state.default_presence_penalty), step=0.01)
		frequency_penalty = st.slider("Frequency Penalty", min_value=-2.0, max_value=2.0, value=float(st.session_state.default_frequency_penalty), step=0.01)
		chat_memory = st.slider("Chat Memory", min_value=0, max_value=10, value=int(st.session_state.default_k_memory), step=1)	
		top_p = st.slider("Top P", min_value=0.0, max_value=1.0, value=float(st.session_state.default_top_p), step=0.01)
		#seed_num = st.slider("Seed Number", min_value=1.0, max_value=100.0, value=float(st.session_state.seed_num), step=1.0)
		...
		if prompt := st.chat_input("Enter your query"):
			...
			with messages.chat_message("assistant"):
				...
				stream = client.chat.completions.create(
					...
					temperature=float(st.session_state.default_temp), #settings option
