    ...
    if document and field_name and document.get(field_name) is None:
        if dict_input is not None:
            ...
            st.session_state.u_collection.update_one(
                {"username": username}, {"$set": {field_name: dict_input}}
            )
    ...
    if document and field_name and document.get(field_name) is None:
        if dict_input is not None:
            ...
            st.session_state.a_collection.update_one(
                {"sch_name": sch}, {"$set": {field_name: dict_input}}
            )
...
def set_prompt_settings():
    ...
    action = st.selectbox(
        "Select Action", ["Edit", "Add", "Remove"], key="app_edit_action"
    )
    new_key = ""
    new_value = ""
    field = ""
...
def perform_modification_and_update_session_state(
    action, field, new_key, new_value, username, excluded_fields
):
    ...
    st.session_state.u_collection.update_one(
        {"username": username}, {"$set": {"prompt_templates": updated_doc}}
    )
...
def manage_prompt_templates():
    ...
    prompt_select = st.selectbox(
        ...
        ["School Prompt Templates", "Personal Prompt Templates"],
    )
    if prompt_select == "School Prompt Templates":
        # st.session_state.current_prompt = prompt_select
        excluded_fields = ["_id", "sch_name"]
        doc = app_load_document(
            st.session_state.user["school_id"], "prompt_templates", PROMPT_CONFIG
        )
        doc_for_df = {k: v for k, v in doc.items() if k not in excluded_fields}
        st.session_state.app_pdf = pd.DataFrame(
            list(doc_for_df.items()), columns=["Field", "Values"]
        ...
        doc = load_document(
            st.session_state.user["id"], "prompt_templates", PROMPT_CONFIG
        )
        # st.session_state.current_prompt = prompt_select
        excluded_fields = ["_id", "username"]
        doc_for_df = {k: v for k, v in doc.items() if k not in excluded_fields}
    ...
    if st.session_state.user["profile_id"] == TCH:
        ...
        if selected_teacher_username == "School Default":
            ...
            teacher_doc = st.session_state.u_collection.find_one(
                {"username": selected_teacher_username}
            )
