...
def rag_creator_mongodb():
    ...
    if st.session_state.user["profile_id"] == SA:
        if st.toggle("Personal RAG"):
            action = st.selectbox(
                "Select action", ["Create RAG", "Display RAG", "Delete RAG"]
            )
    ...
    if rag_list == []:
        ...
        if st.button("Delete"):
            ...
            st.session_state.s_collection.update_one(
                {"sch_name": var}, {"$set": {"rag_list": rag_list}}
            )
            st.session_state.u_collection.update_one(
                {"username": var}, {"$set": {"rag_list": rag_list}}
            )
            st.session_state.r_collection.delete_one(
                {"owner": var, "name": rag_to_delete}
            )
...
def delete_rag_direct():
    st.warning(
        "Use above first, as this action will delete the RAG from the database directly."
    )
    documents = st.session_state.r_collection.find(
        {}, {"owner": 1, "name": 1, "_id": 0}
    )
...
def display_documents_as_dataframe(school):
    ...
    documents_sharing_true = st.session_state.r_collection.find(
...
def create_rag_mongodb(var, school_flag):
    ...
    uploaded_files = st.file_uploader(
        "Choose a file", type=["docx", "txt", "pdf"], accept_multiple_files=True
    )
    new_rag = st.text_input("Enter new RAG name")
    meta = st.text_input(
        "Please enter your document source (Default is MOE):", max_chars=50
    )
    if meta == "":
        meta = "MOE"
    description = st.text_area(
        "Please enter a brief description of the document:", max_chars=200
    )
    if description == "":
...
def sch_check_and_get_rag_list(sch_name):
    school_doc = st.session_state.s_collection.find_one(
        {"sch_name": sch_name}, {"rag_list": 1}
    )
...
def sch_update_rag_list(sch_name, new_value):
    ...
    if rag_list == []:
        st.session_state.s_collection.update_one(
            {"sch_name": sch_name}, {"$set": {"rag_list": [new_value]}}
        )
    elif len(rag_list) < 10:
        rag_list.append(new_value)
        st.session_state.s_collection.update_one(
            {"sch_name": sch_name}, {"$set": {"rag_list": rag_list}}
        )
...
def u_check_and_get_rag_list(u_name):
    school_doc = st.session_state.u_collection.find_one(
        {"username": u_name}, {"rag_list": 1}
    )
    if school_doc and "rag_list" in school_doc:
        return school_doc["rag_list"]
    else:
...
def u_update_rag_list(u_name, new_value):
    ...
    if rag_list == []:
        st.session_state.u_collection.update_one(
            {"username": u_name}, {"$set": {"rag_list": [new_value]}}
        )
    if len(rag_list) < 10:
        rag_list.append(new_value)
        st.session_state.u_collection.update_one(
            {"username": u_name}, {"$set": {"rag_list": rag_list}}
        )
        st.success("Updated successfully!")
    else:
        st.error("No more than 10 items allowed in the list.")
...
def load_rag():
    ...
    if st.toggle("Load Personal RAG"):
        ...
        rag_list = list_rags_for_owner(
            st.session_state.r_collection, st.session_state.user["id"]
        )
        if rag_list == []:
        ...
        if st.session_state.user["profile_id"] == SA:
            ...
            if school != "Select School" and school != None:
                ...
                share_list = list_rags_for_shareable(
                    st.session_state.r_collection, school
                )
                            ...
                            if rag_name != "-":
                                ...
                                if rag_name in share_list:
                                    serialized_faiss = fetch_shared_serialized_faiss(
                                        st.session_state.r_collection, rag_name
                                    )
                                else:
                                    serialized_faiss = fetch_serialized_faiss(
                                        st.session_state.r_collection, rag_name, school
                                    )
            ...
            display_documents_as_dataframe(st.session_state.user["school_id"])
            rag_list = list_rags_for_owner(
                st.session_state.r_collection, st.session_state.user["school_id"]
            )
            share_list = list_rags_for_shareable(
                st.session_state.r_collection, st.session_state.user["school_id"]
            )
