...
def update_app_version():
    key_changes = st.text_area(
        "Key Changes/Improvements", value=CHANGES, key="app_new_prompt", height=400
    )
...
def app_prompt_log_change():
    ...
    tab1, tab2, tab3 = st.tabs(
        ["###### ShortAnsFA", "###### ACP", "###### Prompt Testing App"]
    )
    cwd = os.getcwd()
    WORKING_DIRECTORY = os.path.join(cwd, "database")
    if not os.path.exists(WORKING_DIRECTORY):
    ...
    with tab1:
        st.write(
            "#### :blue[Short Answer Feedback Assistant (ShortAnsFA) Prompt Version Changes]"
        )
        cursor.execute(
            """
			SELECT date, prompt, changes 
