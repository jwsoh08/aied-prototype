...
from basecode2.prompt_log_changes import (
    prompt_change,
    check_update_dates,
    get_last_prompt_change,
)
from basecode2.sqlite_db import (
    create_sql_db,
    insert_condition_value,
    check_condition_value,
    replace_condition_value,
)
...
SAFA = config_handler.get_config_values("constants", "SAFA")
ACP = config_handler.get_config_values("constants", "ACP")
PC = config_handler.get_config_values("constants", "PC")
...
def load_sa_app_settings():
    ...
    if sch_names == []:
        ...
        st.write(
            f"####  :blue[Current Super Admin School: {st.session_state.school_sa_selected}]"
        )
        school = st.selectbox(
            "###### Select School Profile/Settings (Super Admin)",
            ["Select School"] + sch_names,
            key="sa_app_school",
        )
...
def delete_app_settings():
    ...
    if st.session_state.user["profile_id"] != SA:
        ...
        sch_names = sa_select_school()
        school = st.selectbox(
            "Select School", ["Select School"] + sch_names, key="app_school_delete"
        )
        if school:
            if st.checkbox("Are you sure you want to reset app settings?"):
                if st.button("Reset App Configuration"):
                    st.session_state.a_collection.update_one(
                        {"sch_name": school}, {"$set": {"app_settings": APP_CONFIG}}
                    )
...
def delete_prompt_settings():
    ...
    if st.session_state.user["profile_id"] != SA:
        ...
        sch_names = sa_select_school()
        school = st.selectbox(
            "Select School", ["Select School"] + sch_names, key="prompt_school_delete"
        )
        if school:
...
def set_app_settings():
    ...
    if school:
        ...
        st.session_state.app_pdf = pd.DataFrame(
            list(doc_for_df.items()), columns=["Field", "Values"]
        )
        st.write("Current Settings : ", field_name)
        st.dataframe(st.session_state.app_pdf)

        action = st.selectbox("Select Action", ["Edit", "Add"], key="app_edit_action")
        new_key = ""
        new_value = ""
        field = ""
        if action == "Add":
            ...
            field = st.selectbox(
                "Select field",
                ["-"] + st.session_state.app_pdf["Field"].tolist(),
                key="app_action",
            )
            if action == "Edit" and field != "-":
                existing_value = st.session_state.app_pdf.loc[
                    st.session_state.app_pdf["Field"] == field, "Values"
                ].iloc[0]
        ...
        if st.button("Submit Action"):
            ...
            perform_modification_and_update_session_state(
                action, field, new_key, new_value, school, excluded_fields, field_name
            )
            if auto_pro:
                auto_propagate_prompts(field, new_value)
            update_log(field, new_value)
            st.write("Action completed successfully.")


# update log change
@st.experimental_dialog("Update Log Change")
def update_log(field, new_value):
    app_feature = st.text_input("Prompt Name", value=field, key="app_feature")
    new_prompt = st.text_area(
        "New Prompt", value=new_value, key="app_new_prompt", height=400
    )
    new_changes = st.text_area(
        "Highlight Key Changes", key="app_new_changes", height=200
    )
    if st.button(f"Confirmed Update Log Change for {app_feature}"):
        prompt_change(app_feature, new_prompt, new_changes)
        st.success("Log Change Updated Successfully")
        st.rerun()


def perform_modification_and_update_session_state(
    action, field, new_key, new_value, school, excluded_fields, field_name
):
    if action == "Add" and new_key:
        new_row = pd.DataFrame([[new_key, new_value]], columns=["Field", "Values"])
        st.session_state.app_pdf = pd.concat(
            [st.session_state.app_pdf, new_row], ignore_index=True
        )
    elif action == "Edit" and field and field != "-":
        st.session_state.app_pdf.loc[
            st.session_state.app_pdf["Field"] == field, "Values"
        ] = new_value
    elif action == "Remove" and field and field != "-":
        st.session_state.app_pdf = st.session_state.app_pdf[
            st.session_state.app_pdf["Field"] != field
        ]

    updated_doc = {
        row["Field"]: row["Values"]
        for index, row in st.session_state.app_pdf.iterrows()
    }
    st.session_state.a_collection.update_one(
        {"sch_name": school}, {"$set": {field_name: updated_doc}}
    )
...
def propagate_prompts():
    ...
    if st.session_state.user["profile_id"] == SA:
        ...
        source_sch = st.selectbox(
            "Select Source School for Prompts", sch_names, key="source_school"
        )

        # Fetch the prompt templates from the selected source school
        if source_sch:
            source_prompts = fetch_prompts_for_school(
                source_sch
            )  # You need to define this function

            if source_prompts:
                # Now, select target schools for propagation
                selected_schs = st.multiselect(
                    "Select Target Schools",
                    [sch for sch in sch_names if sch != source_sch],
                    key="target_schools",
                )
                ...
                selected_keys = st.multiselect(
                    "Select Prompts to Propagate", prompt_keys, key="propagate_prompts"
                )
                    ...
                    if st.button("Propagate Prompts"):
                        for school in selected_schs:
                            ...
                            for key in selected_keys:
                                ...
                                update_statement["$set"][prompt_path] = source_prompts[
                                    key
                                ]
...
def propagate_settings():
    ...
    if st.session_state.user["profile_id"] == SA:
        ...
        # Select the source school for prompt templates
        source_sch = st.selectbox(
            "Select Source School for Settings", sch_names, key="source_app_school"
        )

        # Fetch the prompt templates from the selected source school
        if source_sch:
            source_prompts = fetch_settings_for_school(
                source_sch
            )  # You need to define this function
            ...
            if source_prompts:
                ...
                selected_schs = st.multiselect(
                    "Select Target Schools",
                    [sch for sch in sch_names if sch != source_sch],
                    key="target_app_schools",
                )

                # List keys from the source prompts for selection
                prompt_keys = list(source_prompts.keys())
                selected_keys = st.multiselect(
                    "Select Settings to Propagate",
                    prompt_keys,
                    key="propagate_settings",
                )
...
def propagate_user_prompts():
    ...
    ):  # Assuming 'SA' is a constant representing a specific profile ID
        # Fetch school names
        sch_names = sa_select_school()

        # Select the source school for prompt templates
        source_sch = st.selectbox(
            "Select Source School for Prompts", sch_names, key="source_school_u"
        )
        ...
        if source_sch:
            ...
            if source_prompts:
                ...
                selected_schs = st.multiselect(
                    "Select Target Schools",
                    [sch for sch in sch_names if sch != source_sch],
                    key="target_schools_u",
                )

                # Select target profiles
                selected_profile = st.selectbox(
                    "Select Profile", SCH_PROFILES, key="target_profile_u"
                )

                # Checkbox to decide if prompts should be updated for all users with the selected profile
                update_all = st.checkbox(
                    "Update for all users with this profile", False
                )

                selected_usernames = []
                if not update_all:
                    # Fetch usernames based on school and profile only if not updating all users with the profile
                    usernames = fetch_usernames_for_school_and_profile(
                        selected_schs, selected_profile
                    )
                    selected_usernames = st.multiselect(
                        "Select Usernames", usernames, key="target_usernames_u"
                    )

                # List keys from the source prompts for selection
                prompt_keys = list(source_prompts.keys())
                selected_keys = st.multiselect(
                    "Select Prompts to Propagate",
                    prompt_keys,
                    key="propagate_prompts_u",
                )

                if (
                    selected_schs
                    and selected_keys
                    and (selected_usernames or update_all)
                ):
                    if st.button("Propagate Prompts"):
                        for school in selected_schs:
                            # Construct the update statement to only modify selected prompts
...
def fetch_usernames_for_school_and_profile(selected_schs, selected_profile):
    ...
    query_result = st.session_state.u_collection.find(
        {"sch_name": {"$in": selected_schs}, "profile": selected_profile},
        {
            "username": 1,
            "_id": 0,
        },  # Projection: Include only the username field and exclude the _id field
    )
