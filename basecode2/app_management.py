...
from basecode2.prompt_log_changes import prompt_change, check_update_dates, get_last_prompt_change
...
def load_sa_app_settings():
	...
	if sch_names == []:
		...
		school = st.selectbox('###### Select School Profile/Settings (Super Admin)', ["Select School"] + sch_names, key='sa_app_school')
...
def set_app_settings():
	...
	if school:
		...
		if field_name == "prompt_templates" and st.session_state.user['profile_id'] == SA:
...
def update_log(field, new_value):
	...
	new_prompt = st.text_area("New Prompt", value=new_value, key='app_new_prompt', height=400)
	new_changes = st.text_area("Highlight Key Changes", key='app_new_changes', height=200)
