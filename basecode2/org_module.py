...
from basecode2.sqlite_db import (
    create_sql_db,
    check_condition_value,
    insert_condition_value,
)
...
def initialise_admin_account():
    ...
    if check_condition_value(ALL_ORG, True):
        ...
        super_admin_exists = st.session_state.u_collection.find_one(
            {"username": secret_retriever.get_secret("super_admin_username")}
        )
            ...
            st.session_state.u_collection.insert_one(
                {
                    ...
                    "password": hash_password(
                        secret_retriever.get_secret("super_admin_username")
                    ),  # hash_password(SUPER_PWD)
                    "profile": SA,
                    "sch_name": ALL_ORG,
                }
            )
...
# -------------------------------------------------------------------------------- Users Function --------------------------------------------------------------------------------
def create_users(
    sch_name, num_users, default_password, profile="", username_prefix=None
):
    ...
    starting_user_id = (
        1 if highest_user_doc is None else highest_user_doc["user_id"] + 1
    )
    ...
    for i in range(starting_user_id, starting_user_id + num_users):
        ...
        hashed_pwd = hash_password(
            default_password
        )  # Assume hash_password is a function you've defined to hash passwords
...
def fetch_usernames_for_school(school_name):
    ...
    users = st.session_state.u_collection.find(
        {"sch_name": school_name}, {"username": 1}
    )
...
def main_delete_users(school_name):
    ...
    action = st.selectbox(
        "Select Action",
        ["Delete All Users", "Delete Users by ID Range", "Delete Selected Users"],
        key="delete_action",
    )
    if action == "Delete All Users":
        ...
        start_id = st.number_input(
            "Start User ID", min_value=0, value=lowid, step=1, key="start_id"
        )
        end_id = st.number_input(
            "End User ID", min_value=0, value=highid, step=1, key="end_id"
        )
        ...
        selected_usernames = st.multiselect(
            "Select users to delete:", options=usernames
        )
...
def fetch_usersid_for_school(school_name):
    ...
    users = list(
        st.session_state.u_collection.find({"sch_name": school_name}, {"user_id": 1})
    )
    return users
...
def delete_users_by_id_range(school_name, start_id, end_id):
    ...
    # Assuming 'users' is your MongoDB collection
    result = st.session_state.u_collection.delete_many(
        {"sch_name": school_name, "user_id": {"$gte": start_id, "$lte": end_id}}
    )
    if result.deleted_count > 0:
        st.success(f"Deleted {result.deleted_count} users successfully.")
    else:
...
def fetch_users_for_school(school_name):
    ...
    users = st.session_state.u_collection.find(
        {"sch_name": school_name}, {"username": 1, "password": 1, "profile": 1}
    )
...
def fetch_users_for_school(school_name):
    ...
    users_cursor = users_cursor = st.session_state.u_collection.find(
        {"sch_name": school_name}, {"username": 1, "password": 1, "profile": 1}
    )
...
def edit_usernames(school_name):
    ...
    for user in users:
        ...
        new_username = st.text_input(
            f"Edit Username for {user['username']}",
            value=user["username"],
            key=user_key,
        )
    ...
    if st.button("Update Usernames"):
        ...
        for user_id, new_username in new_usernames.items():
            if new_username == "super_admin":
                ...
                return
            if not is_unique_username_within_school(
                new_username, school_name, exclude_user_id=user_id
            ):
                st.error(
...
def setup_mass_edit_users(school_name):
    ...
    with st.form("user_edit_form"):
        for user in users_in_school:
            ...
            new_profile = st.selectbox(
                "Select New Profile (Existing profile shown below)",
                SCH_PROFILES,
                index=(
                    SCH_PROFILES.index(current_profile)
                    if current_profile in SCH_PROFILES
                    else 0
                ),
    ...
    if submit_changes:
        ...
        for username, changes in user_changes.items():
            if (
                changes["new_password"] or changes["new_profile"] != current_profile
...
def setup_users():
    ...
    if st.session_state.user["profile_id"] == SA:
        ...
        action = st.selectbox(
            "Select Action",
            ["Edit User", "Remove User", "Create Users", "Edit Usernames"],
            key="u_action",
        )
        if action == "Edit User":
            ...
            action == "Create Users"
        ):  # Step 1: Create a copy of the SCH_PROFILES list to avoid modifying the original
            ...
            num_users = st.number_input(
                "Number of Users to Create", min_value=1, value=1, step=1
            )
            default_profile = st.selectbox(
                "Select Profile", sch_profiles_copy, key="profile"
            )
            default_password = st.text_input(
                "Default Password",
                value=st.session_state.default_password,
                type="password",
            )
            if st.checkbox("Customise Usernames"):
                username_prefix = st.text_input(
                    "Username Prefix", value=school[:3], max_chars=10
                )
            else:
            ...
            if st.button("Create Users"):
                if school and default_password:
                    ...
                    st.error(
                        "Please provide both a school name and a default password."
                    )
    ...
    if st.session_state.user["profile_id"] == SA:
        ...
        if st.button("Add School"):
            if sch_name:
                ...
                if sch_doc:
                    st.error(
                        f"School '{sch_name}' already exists. Please try a different name."
                    )
                else:
                    st.session_state.s_collection.insert_one(
                        {"sch_name": sch_name, "sch_levels": []}
                    )
...
def set_function_access_for_user():
    ...
    user_doc = st.session_state.u_collection.find_one(
    ...
    sch_doc = st.session_state.s_collection.find_one(
    ...
    if not function_list:  # If the profile doesn't exist or has no functions
        st.session_state.func_options = {
            key: True for key in MENU_FUNCS.keys()
        ...
        st.session_state.func_options = {
            func: not function_list.get(func, True) for func in MENU_FUNCS.keys()
        }
...
def edit_function(sch_name):
    ...
    with st.container():
        ...
        for func, desc in MENU_FUNCS.items():
            ...
            st.session_state.func_options[func] = not st.checkbox(
                f"{func}: {desc}", value=current_value, key=func
            )

    if st.button("Save Profile Functions"):
        # Invert the values back before saving to database
        func_options_to_save = {
            func: not value for func, value in st.session_state.func_options.items()
        }
        ...
        updated_school_doc = st.session_state.s_collection.find_one(
            {"sch_name": sch_name}
        )
        ...
        matching_users = st.session_state.u_collection.find(
            {"sch_name": sch_name, "profile": profile_name}
        )

        for user in matching_users:
            # Also, invert the profile functions for consistency
            user_profile_functions_to_save = {
                func: not value for func, value in updated_profile_functions.items()
            }
            st.session_state.u_collection.update_one(
                {"_id": user["_id"]},
        ...
        st.success(
            f"All matching user documents for profile '{profile_name}' in school '{sch_name}' have been updated with new profile functions."
        )
...
def manage_levels_classes(sch_name):
    ...
    action = st.selectbox(
        "Select Action",
        ["Add Level", "Remove Level", "Add/Remove Classes"],
        key="c_action",
    )
    if action == "Add Level":
        add_level(sch_name)
...
def add_level(sch_name):
    ...
    if level_name:
        ...
        if not level_name.startswith("lvl_") or not all(
            char.isalnum() or char == "_" for char in level_name[4:]
        ):
            st.error(
                "Invalid level name. It must start with 'lvl_' and contain only alphanumeric characters or underscores."
            )
            return
        ...
        if level_name in sch_doc:
            st.error(
                f"Level '{level_name}' already exists in '{sch_name}'. Consider editing the classes or removing the level."
            )
            # Optionally, display existing classes for the level
            existing_classes = sch_doc.get(level_name, [])
            st.write(
                f"Existing classes for {level_name}: {', '.join(existing_classes)}"
            )
            return
        ...
        if st.button("Add Level"):
            ...
            st.session_state.s_collection.update_one(
                {"sch_name": sch_name}, update_action, upsert=True
            )
...
def add_remove_classes(sch_name):
    ...
    if st.button("Add Class", key="add_class") and new_class:
        if new_class not in classes:
            ...
            st.session_state.s_collection.update_one(
                {"sch_name": sch_name}, {"$set": {level_name: classes}}
            )
    ...
    if sch_name:
        ...
        if sch_doc and "sch_levels" in sch_doc:
            ...
            level_to_remove = st.selectbox(
                "Select Level to Remove", ["Choose a level"] + levels
            )
            ...
            if level_to_remove != "Choose a level":
                ...
                if st.button(f"Remove '{level_to_remove}' Level"):
                    ...
                    update_action = {
                        "$pull": {
                            "sch_levels": level_to_remove
                        },  # Remove from sch_levels list
                        "$unset": {level_to_remove: ""},  # Remove the level field
                    }
                    st.session_state.s_collection.update_one(
                        {"sch_name": sch_name}, update_action
                    )
                    st.success(
                        f"Level '{level_to_remove}' successfully removed from '{sch_name}'."
                    )
        else:
            st.error(
                "No levels found for this school, or school does not exist. Please check the school name."
            )
    else:
...
def sa_delete_profile_from_school():
    st.subheader(
        ":red[Delete current function access settings for a profile in a school]"
    )
    ...
    if st.session_state.user["profile_id"] == SA:
        ...
        if school:
            profile_to_delete = st.selectbox(
                "Select Profile to Delete", SCH_PROFILES, key="profile_to_delete"
            )
...
def delete_profile(sch_name, profile_var):
    ...
    if profile_var in sch_doc:
        ...
        result = st.session_state.s_collection.update_one(
            {"sch_name": sch_name}, {"$unset": {profile_var: ""}}
        )

        if result.modified_count > 0:
            st.success(
                f"Profile '{profile_var}' deleted successfully from '{sch_name}'."
            )
        else:
            st.warning(
                f"Profile '{profile_var}' could not be deleted or did not exist in '{sch_name}'."
            )
    else:
    ...
    if st.session_state.user["profile_id"] == SA:
        ...
        if school:
            ...
            with c1:
                generate_full_structure(
                    school, st.session_state.s_collection, st.session_state.u_collection
                )
        ...
        with c1:
            generate_full_structure(
                ...
                st.session_state.s_collection,
                st.session_state.u_collection,
            )
        ...
        students_cursor = st.session_state.u_collection.find(
            {
                ...
                "level": {
                    "$in": [level_name]
                },  # Check if 'level_name' is in the list of levels
                "class": {
                    "$in": [class_name]
                },  # Check if 'class_name' is in the list of classes
...
def manage_teachers(sch_name):
    ...
    action = st.selectbox(
        "Select Action", ["Add Teachers", "Remove Teachers"], key="teacher_action"
    )
        ...
        if not selected_teachers:
            ...
            if st.button("Remove Teachers"):
                for teacher in selected_teachers:
                    # Fetch the teacher's document
                    teacher_doc = st.session_state.u_collection.find_one(
                        {"username": teacher}
                    )
                    ...
                    update_result = st.session_state.u_collection.update_one(
                        ...
                        {
                            "$set": {
                                "class": teacher_doc["class"],
                                "level": teacher_doc["level"],
                            }
                        },
                    )
                    ...
                    if update_result.modified_count > 0:
                        st.success(
                            f"Removed one instance of '{level_name}' and '{class_name}' from '{teacher}'."
                        )
                    else:
                        st.error(
                            "Failed to update the teacher's information. Please try again."
                        )
...
def add_teachers_to_class(sch_name, level_name, class_name):
    ...
    if all_teachers == []:
        ...
        num_teachers = NUM_TCH - len(
            fetch_teachers_for_class(sch_name, level_name, class_name)
        )
        if num_teachers == 0:
            ...
            if not selected_teachers:
                ...
                if st.button("Add Teachers"):
                    if num_teachers < len(selected_teachers):
                        st.warning(
                            f"Number of teachers to add ({len(selected_teachers)}) exceeds the available slots ({num_teachers})."
                        )
                        return
                    for teacher in selected_teachers:
                        ...
                        already_assigned = (
                            st.session_state.u_collection.count_documents(
                                {
                                    "username": teacher,
                                    "level": level_name,
                                    "class": class_name,
                        ...
                        if already_assigned > 0:
                            st.warning(
                                f"{teacher} is already assigned to {level_name} - {class_name}."
                            )
                            continue
                    ...
                    st.success(
                        f"Successfully added {len(selected_teachers)} teachers to the class."
                    )
...
def fetch_all_teachers(sch_name):
    ...
    teachers_cursor = st.session_state.u_collection.find(
        {"sch_name": sch_name, "profile": TCH}, {"_id": 0, "username": 1}
    )
...
def manage_students(sch_name):
    ...
    st.write(f":green[Selected: {level_name} - {class_name}]")
    action = st.selectbox(
        "Select Action", ["Add Students", "Remove Students"], key="student_action"
    )
    ...
    if students == []:
        ...
        if not selected_students:
            ...
            if st.button("Remove Students"):
                ...
                st.success(
                    f"Successfully removed {len(selected_students)} students from the class."
                )
...
def add_students_to_class(sch_name, level_name, class_name):
    ...
    if not all_students:
        st.warning(
            f"No unassigned students found for {sch_name}. Please add or remove students from classes first."
        )
        return
    else:
        ...
        num_students = NUM_STU - len(
            fetch_students_for_class(sch_name, level_name, class_name)
        )
        st.write(
            f"Number of students that can be added to {level_name} - {class_name} is {num_students}"
        )
        ...
        if not selected_students:
            ...
            if st.button("Add Students"):
                ...
                if len_selected_students > num_students:
                    st.error(
                        f"Number of students selected ({len_selected_students}) exceeds the available slots ({num_students})."
                    )
                    return
                else:
                    ...
                    st.success(
                        f"Successfully added {len_selected_students} students to {level_name} - {class_name}."
                    )
